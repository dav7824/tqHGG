// Making stacked plots for histograms after MVA reconstruction of TT signal in hadronic channel. (QCD samples are not included)
// Signal MC histograms are also drawn. Each signal MC histogram is normalized to have the same yield as the corresponding data histogram.
//
...
// arguments:
// 1. MVA method
// 2. MVA score value cut
// 3. (default = 0) whether use log scale for y axis
// 4. (default = 0) whether calculate event yields

void plotter_MVAreco_TThad_v3(TString method, TString cut_MVA, bool use_logy = 0, bool calcYield = 0)
{
    ...
    cout << "---Openning file: " << sample << endl;
    TFile *fin_sigMC = new TFile(indir + sample + "_blind.root");
    TFile *fin_sigMC_unblind = new TFile(indir + sample + "_unblind.root");
	...
	if ( TString(key->GetName()) == "dipho_mass" ) hsig.push_back( (TH1D*)fin_sigMC_unblind->Get( "dipho_mass" ) );
	else hsig.push_back( (TH1D*)fin_sigMC->Get( key->GetName() ) );
    }
	...
	// t+W
	hbkg[5][i]->Add(hbkg[6][i]);
	// tt(gamma)+jets
	hbkg[8][i]->Add(hbkg[9][i]);
	// tt+V
	hbkg[10][i]->Add(hbkg[11][i]);
	// VG
	hbkg[13][i]->Add(hbkg[14][i]);
	// VV
	hbkg[15][i]->Add(hbkg[16][i]);
	hbkg[15][i]->Add(hbkg[17][i]);
	// SM Higgs
	hbkg[18][i]->Add(hbkg[19][i]);
	hbkg[18][i]->Add(hbkg[20][i]);
	hbkg[18][i]->Add(hbkg[21][i]);
	hbkg[18][i]->Add(hbkg[22][i]);
	hbkg[18][i]->Add(hbkg[23][i]);
    }
    ...
    */
	...
	if ( TString( hsig[i]->GetName() ) == "dipho_mass" ) hsig[i]->Scale( 0.5 * hdata[i]->Integral()/hsig[i]->Integral() );
	else hsig[i]->Scale( hdata[i]->Integral()/hsig[i]->Integral() );
	// set y maxima for plot 1
	double ymax = hdata[i]->GetMaximum();
	if (ymax < hbkg_tot[i]->GetMaximum()) ymax = hbkg_tot[i]->GetMaximum();
	if (ymax < hsig[i]->GetMaximum()) ymax = hsig[i]->GetMaximum();
	if (!use_logy)  hstack[i]->SetMaximum(ymax * 1.4);
	else hstack[i]->SetMaximum(ymax * 1000);

...
void MggStackedPlot_had()
{
    ...
    // TT_hct
    hsig["TT_hct"] = (TH1D*)hsample["TT_FCNC-TtoHJ_aThad_hct"]->Clone();
    hsig["TT_hct"]->Add( hsample["TT_FCNC-aTtoHJ_Thad_hct"] );
    // TT_hut
    hsig["TT_hut"] = (TH1D*)hsample["TT_FCNC-TtoHJ_aThad_hut"]->Clone();
    hsig["TT_hut"]->Add( hsample["TT_FCNC-aTtoHJ_Thad_hut"] );
    // ST_hct
    hsig["ST_hct"] = (TH1D*)hsample["ST_FCNC-TH_Thad_hct"]->Clone();
    // ST_hut
    hsig["ST_hut"] = (TH1D*)hsample["ST_FCNC-TH_Thad_hut"]->Clone();
    // DiPhotonJetsBox
    hbkg["DiPhotonJetsBox"] = (TH1D*)hsample["DiPhotonJetsBox_scaled"]->Clone();
    // Data-driven GJet_QCD
    hbkg["GJet_QCD"] = (TH1D*)hsample["Data-driven_QCD"]->Clone();
    // TTGG
    ...
    hbkg["Others"]->Add( hsample["DYJetsToLL"] );
    hbkg["Others"]->Add( hsample["WW"] );
    hbkg["Others"]->Add( hsample["WZ"] );
    hbkg["Others"]->Add( hsample["ZZ"] );
    // Total bkg
    TH1D *htotbkg = (TH1D*)hsample["DiPhotonJetsBox_scaled"]->Clone();
    htotbkg->Add( hsample["Data-driven_QCD"] );
    ...
    c->SaveAs("dipho_mass_blind_had.png");

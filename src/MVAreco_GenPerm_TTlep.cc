/*
 * Producing permutation trees for leptonic TT signal reconstruction.
 * Usage:
 *   ./MVAreco_GenPerm_TTlep <fin> <fout> <sig|bkg>
 *
 *
 ...
 *
 * For sig mode, events with "EvtInfo.EvtNo%3 == 0" are intended for training reconstruction MVA. The
 * perms chosen would have "Perm_isTrain" values as 1.
 *
...
int main(int argc, char **argv)
{
    ...
    for (int evt=0; evt<Nevt_tot; ++evt)
    {
        ...
        // Event that can't be reconstructed are saved as null to output
        if (jets_size < 2) {
            Tout->Fill();
            continue;
        }
        ...
        Perm_isTrain = is_signal && (Evt_no % 3 == 0);
        // Calculate total number of perms for this event
        Perm_N = jets_size * (jets_size-1) * Lep_Size;

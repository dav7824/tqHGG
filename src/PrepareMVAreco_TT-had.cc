// Prepare datasets for MVA reconstruction of ttbar signal, in hadronic channel, using 2017 background MC & data.
//
// input arguments:
...
int main(int argc, char **argv)
{
	...
	for (int n=0; n<idx_bjets.size(); ++n) { // loop for b-jet from M2
	    for (int i=0; i<idx_light_jets.size(); ++i) { // loop for light jet from M1
		for (int j=0; j<idx_light_jets.size()-1; ++j) { // loop for light jet 1 from W
		    if (j == i) continue;
		    for (int k=j+1; k<idx_light_jets.size(); ++k) { // loop for light jet 2 from W
			if (k == i) continue;
			...
			idx_perm += 1;
		    } // end of loop for light jet 2 from W
		} // end of loop for light jet 1 from W
	    } // end of loop for light jet from M1
	} // end of loop for b-jet from M2

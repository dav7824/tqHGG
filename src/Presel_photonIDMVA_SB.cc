// Preselection:
//   Select the events in the photon IDMVA side-band.
//   Side-band region: leading photon IDMVA >= -0.7 and -0.7 > subleading photon IDMVA > -0.9
...
int main(int argc, char **argv)
{
	...
	for (int evt=0; evt<inTree->GetEntries(); ++evt)
	{
		...
		if (DiPhoInfo_leadIDMVA < -0.7) continue;
		if (DiPhoInfo_subleadIDMVA >= -0.7 || DiPhoInfo_subleadIDMVA <= -0.9) continue;

 ...
 * Usage:
 *   ./PrintYieldSummary <bkg_file_list> <bkg_name_list> <bkg_dir> <data.root> <data_tree> <summary.txt>
 */
...
int main(int argc, char **argv)
{
	...
	TString tree_name = argv[5];    // Name of data tree
	TString fsummary_name = argv[6];     // Path of summary text file
	...
	float yield_data = (float)((TTree*)fdata->Get(tree_name))->GetEntries();
	...
	for (int i=0; i<Nbkgs; ++i) {
		fprintf(fsummary, format, samples[i].Data(), yield_bkg[i][0], sqrt(yield_bkg[i][1]));
	}
	fprintf(fsummary, format, "Total MC bkg", yield_bkg_tot, yield_bkg_tot_err);
	fprintf(fsummary, format, "Data", yield_data, yield_data_err);
	fprintf(fsummary, format, "(MC-data) / data", ratio, ratio_err);
	fclose(fsummary);

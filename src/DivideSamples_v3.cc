/*
 * Divide trees using tree variable "EvtInfo.EvtNo".
 * The first tree has to contain that branch.
 *
 * Usage:
 *   ./DivideSamples_v3 <fin> <vec_tree> <vec_fout>
 */
...
int main(int argc, char **argv)
{
    ...
    // Set branch
    int EvtInfo_EvtNo = 0;
    Tin[0]->SetBranchAddress("EvtInfo.EvtNo", &EvtInfo_EvtNo);
    ...
    for (int ifout=0; ifout<nfout; ++ifout)
        cout << "[INFO] Saved output file " << ifout << ": " << fout_name[ifout] << endl;

...
void FillHist(TreeReader &inTree, TH1D **hist, const string &type)
{
	...
	hist[jet_N]->Fill(inTree.jets_size, weight);
	for (int j=0; j<inTree.jets_size; ++j) {
	    hist[jet_pt]->Fill(inTree.JetInfo_Pt->at(j), weight);
	    hist[jet_eta]->Fill(inTree.JetInfo_Eta->at(j), weight);
	    hist[jet_phi]->Fill(inTree.JetInfo_Phi->at(j), weight);
	    hist[jet_E]->Fill(inTree.JetInfo_Energy->at(j), weight);
	    hist[jet_M]->Fill(inTree.JetInfo_Mass->at(j), weight);
	}

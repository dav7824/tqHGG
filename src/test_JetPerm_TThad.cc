...
int main(int argc, char **argv)
{
	...
	TFile *fin = new TFile(dir+"TT_FCNC-TtoHJ_aThad_hct.root");
	TTree *T = (TTree*)fin->Get("T");
	T->AddFriend("TGenPart");

	TFile *fout = new TFile(dir+"test_important_jet/TT_FCNC-TtoHJ_aThad_hct.root", "recreate");
	...
	TLorentzVector bq, lq, Wq1, Wq2;
	TLorentzVector bjet, ljet, Wjet1, Wjet2;
			...
			for (int j=0; j<njet_; ++j) {
				...
				for (int k=0; k<njet_-1; ++k) {
					...
					for (int l=k+1; l<njet_; ++l) {
						...
						if (bjet.DeltaR(bq)<dR_cut && ljet.DeltaR(lq)<dR_cut && W_matched
